...
import Referral from './referral.model.js'; // Use .js
...
export const createReferralService = async (payload: Partial<IReferral>): Promise<IReferral> => {
  try {
    const result = await Referral.create(payload);
    return result;
  } catch (error: any) {
    ...
    throw new AppError(`Failed to create referral: ${error.message}`, httpStatus.INTERNAL_SERVER_ERROR);
  }
};
...
export const getAllReferralsService = async (
...
): Promise<IGenericDataResponse<IReferral[]>> => {
  ...
  const [result, total] = await Promise.all([
    Referral.find(whereCondition)
      .sort(sortCondition)
      .skip(skip)
      .limit(limit)
      // Example population: .populate('createdBy', 'name email')
      .lean(), // Use lean for performance if not modifying docs
    Referral.countDocuments(whereCondition),
  ]);
...
export const getReferralByIdService = async (id: string): Promise<IReferral | null> => {
  if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError('Invalid ID format', httpStatus.BAD_REQUEST);
  }
  const result = await Referral.findById(id); //.populate('...');
  if (!result) {
        throw new AppError('Referral not found', httpStatus.NOT_FOUND);
  }
  return result;
};
...
export const updateReferralService = async (
  ...
  const result = await Referral.findByIdAndUpdate(id, payload, {
    new: true, // Return the updated document
    runValidators: true, // Run schema validators on update
  });
   if (!result) {
        throw new AppError('Referral not found or update failed', httpStatus.NOT_FOUND);
  }
  return result;
};

// Example Service: Delete document by ID
export const deleteReferralService = async (id: string): Promise<IReferral | null> => {
   if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError('Invalid ID format', httpStatus.BAD_REQUEST);
   }
