...
import Auth from './auth.model.js'; // Use .js
...
export const createAuthService = async (payload: Partial<IAuth>): Promise<IAuth> => {
  try {
    ...
    throw new AppError(`Failed to create auth: ${error.message}`, httpStatus.INTERNAL_SERVER_ERROR);
  }
};

// Example Service: Get all documents with pagination
export const getAllAuthsService = async (
  paginationOptions: IPaginationOptions
): Promise<IGenericDataResponse<IAuth[]>> => {
  ...
  const [result, total] = await Promise.all([
    Auth.find(whereCondition)
      .sort(sortCondition)
      .skip(skip)
      .limit(limit)
      // Example population: .populate('createdBy', 'name email')
      .lean(), // Use lean for performance if not modifying docs
    Auth.countDocuments(whereCondition),
  ]);
...
export const getAuthByIdService = async (id: string): Promise<IAuth | null> => {
  if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError('Invalid ID format', httpStatus.BAD_REQUEST);
  }
  const result = await Auth.findById(id); //.populate('...');
  if (!result) {
        throw new AppError('Auth not found', httpStatus.NOT_FOUND);
  }
  return result;
};
...
export const updateAuthService = async (
  ...
  const result = await Auth.findByIdAndUpdate(id, payload, {
    new: true, // Return the updated document
    runValidators: true, // Run schema validators on update
  });
   if (!result) {
        throw new AppError('Auth not found or update failed', httpStatus.NOT_FOUND);
  }
  return result;
};

// Example Service: Delete document by ID
export const deleteAuthService = async (id: string): Promise<IAuth | null> => {
   if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError('Invalid ID format', httpStatus.BAD_REQUEST);
   }
