// src/app/modules/PromoCode/promoCode.validation.ts
import { z } from 'zod';

// Example: Basic validation for creation
const createPromoCodeValidationSchema = z.object({
  body: z.object({
    name: z.string({ required_error: 'PromoCode name is required' }).min(1),
...
const updatePromoCodeValidationSchema = createPromoCodeValidationSchema.deepPartial();
// You might need more specific update schemas depending on which fields are allowed

export const PromoCodeValidation = {
  createPromoCodeValidationSchema,
  updatePromoCodeValidationSchema,
};
