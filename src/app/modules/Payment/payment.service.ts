...
import Payment from './payment.model.js'; // Use .js
...
export const createPaymentService = async (payload: Partial<IPayment>): Promise<IPayment> => {
  try {
    ...
    throw new AppError(`Failed to create payment: ${error.message}`, httpStatus.INTERNAL_SERVER_ERROR);
  }
};
...
export const getAllPaymentsService = async (
...
): Promise<IGenericDataResponse<IPayment[]>> => {
  ...
  const [result, total] = await Promise.all([
    Payment.find(whereCondition)
      .sort(sortCondition)
      .skip(skip)
      .limit(limit)
      // Example population: .populate('createdBy', 'name email')
      .lean(), // Use lean for performance if not modifying docs
    Payment.countDocuments(whereCondition),
  ]);
...
export const getPaymentByIdService = async (id: string): Promise<IPayment | null> => {
...
export const updatePaymentService = async (
  ...
  const result = await Payment.findByIdAndUpdate(id, payload, {
   ...
   if (!result) {
        throw new AppError('Payment not found or update failed', httpStatus.NOT_FOUND);
  }
  return result;
};

// Example Service: Delete document by ID
export const deletePaymentService = async (id: string): Promise<IPayment | null> => {
   if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new AppError('Invalid ID format', httpStatus.BAD_REQUEST);
   }
