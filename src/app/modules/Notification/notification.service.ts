...
import Notification from './notification.model.js'; // Use .js
...
export const createNotificationService = async (payload: Partial<INotification>): Promise<INotification> => {
  try {
    ...
    throw new AppError(`Failed to create notification: ${error.message}`, httpStatus.INTERNAL_SERVER_ERROR);
  }
};
...
export const getAllNotificationsService = async (
...
): Promise<IGenericDataResponse<INotification[]>> => {
  ...
  const [result, total] = await Promise.all([
    Notification.find(whereCondition)
      .sort(sortCondition)
      .skip(skip)
      .limit(limit)
      // Example population: .populate('createdBy', 'name email')
      .lean(), // Use lean for performance if not modifying docs
    Notification.countDocuments(whereCondition),
  ]);
...
export const getNotificationByIdService = async (id: string): Promise<INotification | null> => {
  ...
  const result = await Notification.findById(id); //.populate('...');
  if (!result) {
...
export const updateNotificationService = async (
  ...
  const result = await Notification.findByIdAndUpdate(id, payload, {
   ...
   if (!result) {
        throw new AppError('Notification not found or update failed', httpStatus.NOT_FOUND);
  }
  return result;
};
