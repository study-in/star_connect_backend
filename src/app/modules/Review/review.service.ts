...
import Review from './review.model.js'; // Use .js
...
export const createReviewService = async (payload: Partial<IReview>): Promise<IReview> => {
  try {
    ...
    throw new AppError(`Failed to create review: ${error.message}`, httpStatus.INTERNAL_SERVER_ERROR);
  }
};
...
export const getAllReviewsService = async (
...
): Promise<IGenericDataResponse<IReview[]>> => {
  ...
  const [result, total] = await Promise.all([
    Review.find(whereCondition)
      .sort(sortCondition)
      .skip(skip)
      .limit(limit)
      // Example population: .populate('createdBy', 'name email')
      .lean(), // Use lean for performance if not modifying docs
    Review.countDocuments(whereCondition),
  ]);
...
export const updateReviewService = async (
  ...
  const result = await Review.findByIdAndUpdate(id, payload, {
   ...
   if (!result) {
        throw new AppError('Review not found or update failed', httpStatus.NOT_FOUND);
  }
  return result;
};
